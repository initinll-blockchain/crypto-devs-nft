<script lang="ts">
	...
	import { checkIfWalletIsConnected, connectWallet, getNetwork, 
        switchNetwork, startPresale, presaleMint, postsaleMint,
        getTotalTokenIdsMinted, isPresaleStarted, isPresaleEnded } from '$lib/services/CryptoDevsService';	
      ...
      <div>
        ...
        {#if account}
            ...
            {:else if isOwner && !presaleStarted}
                <button class="button" on:click={startPresale}>
                    Start Presale!
                </button>
            ...
            {:else if presaleStarted && !presaleEnded}
                <div>
                    ...
                    <button class="button" on:click={presaleMint}>
                        Presale Mint 🚀
                    </button>
                </div>
            {:else if presaleStarted && presaleEnded}
                <button class="button" on:click={postsaleMint}>
                    Public Mint 🚀
                </button>
            {/if}
        {:else}
